...
avg_gp = avg_merge[,c("GenotypeID", "TotalGrainMassGrams", "Total.Weight.of.kernels..g.")]
colnames(avg_gp) = c("GenotypeID", "Yield_NE", "Yield_MI")
...
write.table(full_train_result,"data2/rrblupresult/rrblup_full.csv",quote = F,row.names = F,sep = ",")
# 
...
for (t in 1:20) {
  ...
  # correlations
  MIyield = avg_gp_bs$Yield_MI
  NEyield = avg_gp_bs$Yield_NE
  GPyield = full_train_result_bs[[t]]$Yield_NE
  yield_comparisons = data.frame(MIyield=MIyield,NEyield=NEyield,GPyield=GPyield)
  write.table(yield_comparisons,paste0("data2/rrblupresult/NE_rrblupfullbs.csv",t),quote=F,col.names = T,row.names = F, sep=",")
...
#   }
#   write.table(predicted_cv_list[[t]],paste0("data2/rrblupresult/NE_rrblupcv.csv",t),quote=F,col.names = T,row.names = F,sep = ",")
#   # correlations
