...
corr_gp_full = c()
for (t in 1:10) {
  avg_gp_bs = avg_gp[sample(1:g_number,as.integer(0.8*g_number)),]
  full_train_result_bs[t]=avg_gp_bs
  for (i in 2:ncol(avg_gp)) {
    full_train_result_bs[[t]][i] = rrblup_full(avg_gp_bs, i, snp_markers2)
  }
  write.table(full_train_result_bs[[t]],paste0("data/rrblupresult/2020NEMI_rrblup_full.csv",t),quote = F,row.names = F,sep = ",")
  # correlations
...
predicted_cv_result=list()
corr_gp_cv = c()
corr_gp_cvNE = c()
for (t in 1:10) {
  predicted_cv_result[t]=avg_gp
  for (i in 2:ncol(avg_gp)) {
    predicted_cv_result[[t]][i] = rrblup_cv(avg_gp_bs, i, snp_markers2, 5)
  }
  write.table(predicted_cv_result[[t]],paste0("data/rrblupresult/2020NEMI_rrblupcv.csv",t),quote=F,col.names = T,row.names = F)
  # correlations
  MIyield = avg_gp$Yield_MImean
  NEyield = avg_gp$TotalGrainMassGrams_NEmean
  GPyield = predicted_cv_result[[t]]$TotalGrainMassGrams_NEmean
