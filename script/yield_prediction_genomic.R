...
# read marker
snp_markers  = read.csv("WiDiv_maf005_725geno.num.txt")
...
# }
# write.table(full_train_result,"data/rrblupresult/2020NEMI_rrblup_full.csv",quote = F,row.names = F,sep = ",")
# 
...
#for (t in 1:10) {
  ...
  write.table(full_train_result_bs[[t]],paste0("data/rrblupresult/2020NEMI_rrblupfull_bs.csv",t),quote = F,row.names = F,sep = ",")
  # correlations
  ...
  write.table(yield_comparisons,paste0("data/rrblupresult/2020NEMI_rrblupfull_comparisons.csv",t),quote=F,col.names = T,row.names = F)
...
#for (t in 1:10) {
  ...
  for (i in 2:ncol(avg_gp)) {
    predicted_cv_list[[t]][,i] = rrblup_cv(avg_gp, i, snp_markers2, 5)
  }
  write.table(predicted_cv_list[[t]],paste0("data/rrblupresult/2020NEMI_rrblupcv.csv",t),quote=F,col.names = T,row.names = F)
  # correlations
  ...
  yield_comparisons = data.frame(MIyield=MIyield,NEyield=NEyield,GPyield=GPyield)
  write.table(yield_comparisons,paste0("data/rrblupresult/2020NEMI_rrblupcv_comparisons.csv",t),quote=F,col.names = T,row.names = F)
